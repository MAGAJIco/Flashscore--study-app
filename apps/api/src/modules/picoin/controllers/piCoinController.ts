...
import { PiWallet } from '../../../models/PiWallet';
...
export const piCoinController = {
  ...
  async getTransactions(req: FastifyRequest<{ Params: { userId: string }; Querystring: { limit?: string } }>, res: FastifyReply) {
    try {
      const { userId } = req.params;
  ...
  // Get leaderboard
  async getLeaderboard(req: FastifyRequest<{ Querystring: { limit?: string } }>, res: FastifyReply) {
    try {
      ...
      const leaderboard = await PiWallet.find()
        .sort({ balance: -1 })
        .limit(limit)
        .select('userId balance level totalEarned');
  ...
  // Purchase coins (for Pi Network integration)
